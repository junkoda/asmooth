...
void for_each_file(const int file_index,
		   ...
		   //float* const mesh,
		   Histogram* const hist, Histogram* const hist_halo,
		   deque<int>& ncs);

void calc_pdf(Particles const * const particles,
	      Histogram* const hist, Histogram* const hist_halo,
	      const int nc);

void write_pdf(const char filename[],
	       Histogram const * const hist,
	       Histogram const * const hist_halo,
	       const int nc);
...
int main(int argc, char* argv[])
{
  ...
  //assert(mesh);
  Histogram* hist= new Histogram[nc*nc*nc];
  Histogram* hist_halo= new Histogram[nc*nc*nc];
  for(int j=0; j<nc*nc*nc; ++j) {
    hist[j].initialize(log(0.001f), log(1000.0f), 700);
    hist_halo[j].initialize(log(0.001f), log(1000.0f), 700);
  }
		   ...
		   deque<int>& ncs)
{
  ...
  logger << "z " << redshift << "\n";
  char filename[256];
  const float shift[]= {0.0f, 0.0f, 0.0f};
...
#pragma omp parallel for private(knbrs)
  ...
  for(deque<int>::iterator p= ncs.begin(); p != ncs.end(); ++p) {
    ...
    calc_pdf(particles, hist, hist_halo, nc);
    
    sprintf(filename, "fof/nc%d/%spdf%d.dat", nc, redshift, file_index);
    write_pdf(filename, hist, hist_halo, nc);
...
#pragma omp parallel for reduction(+:np_halo, m_halo)
  ...
  for(deque<int>::iterator p= ncs.begin(); p != ncs.end(); ++p) {
    ...
    sprintf(filename, "so/nc%d/%spdf%d.dat", nc, redshift, file_index);
    write_pdf(filename, hist, hist_halo, nc);
	      ...
	      const int nc)
{
  ...
  // debug: this parallelism should be OK
#ifdef _OPENMP
#pragma omp parallel for shared(hist, hist_halo) reduction(+: np_inbox)
#endif
  for(index_t j=0; j<np; ++j) {
    ...
    int ii[3];
    ii[0]= (int)floor(x[0]/boxsize * nc);
    ii[1]= (int)floor(x[1]/boxsize * nc);
    ii[2]= (int)floor(x[2]/boxsize * nc);
    ...
    if(0 <= ii[0] && ii[0] < nc &&
      ...
      int index= ii[0] + ii[1]*nc + ii[2]*nc2;
      assert(0<=index && index < nhist);
      
      // density is per internal volume
      // mean density is 1/8
      if(p[j].igrp <= 0)
	hist[index].add(log(8.0f*p[j].dens));
      else
	hist_halo[index].add(log(8.0f*p[j].dens));
    }
  }
  cerr << "np_inbox " << np_inbox << endl;
}
	       ...
	       const int nc)
{
  ...
  hist[0].write_header(fp);

  for(int j=0; j<nc*nc*nc; ++j) {
    fwrite(&j, sizeof(int), 1, fp);
    hist[j].write(fp);
    hist_halo[j].write(fp);
  }
